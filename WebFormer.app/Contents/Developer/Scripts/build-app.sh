...
BUNDLE_IDENTIFIER="com.subtlegradient.webformer"
VERSION="0.1.0"
...
APP="${WEBFORMER_APP_OUTPUT:-$DEVELOPER_ROOT/dist/$APP_NAME.app}"
SIGN_ADHOC="${WEBFORMER_APP_SIGN:-1}"
...
if [[ ! -x "$APP_SERVER_SOURCE/main.sh" ]]; then
  echo "Missing WebFormer app server at $APP_SERVER_SOURCE" >&2
  exit 1
fi

if [[ ! -f "$RUNNER_SOURCE/package.json" ]]; then
  echo "Missing WebFormer runner source at $RUNNER_SOURCE" >&2
  exit 1
fi

if [[ ! -d "$DEVELOPER_SOURCE/Scripts" ]]; then
  echo "Missing WebFormer developer scripts at $DEVELOPER_SOURCE/Scripts" >&2
  exit 1
fi

STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/webformer-build.XXXXXX")"
cleanup() {
  rm -rf "$STAGING_ROOT"
}
trap cleanup EXIT
...
<dict>
  ...
  <dict>
    ...
    <string>com.subtlegradient.webform</string>
    <key>LogName</key>
    <string>WebFormer</string>
  ...
  <array>
    <dict>
      ...
      <string>WebForm Document</string>
...
Contents/Resources/Runner/.webform-test/
