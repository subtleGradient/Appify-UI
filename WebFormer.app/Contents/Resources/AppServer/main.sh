...
RUNNER_DIR="${WEBFORMER_RUNNER_DIR:-$APP/Contents/Resources/Runner}"
...
write_nix_bun_wrapper() {
  ...
  local wrapper_dir="$cache_root/Appify-UI/WebFormer"
...
if ! BUN_PATH="$(resolve_bun)"; then
  printf 'WebFormer requires Bun. Install it from https://bun.sh, install Nix, or set APPIFY_HOST_BUN.\n' >&2
  exit 1
fi
