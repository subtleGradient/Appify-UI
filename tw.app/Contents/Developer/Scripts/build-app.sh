#!/usr/bin/env bash
set -euo pipefail

APP_NAME="tw"
EXECUTABLE_NAME="appify-host"
BUNDLE_IDENTIFIER="com.subtlegradient.tw"
VERSION="0.1.0"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DEVELOPER_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SOURCE_CONTENTS="$(cd "$DEVELOPER_ROOT/.." && pwd)"
SOURCE_APP="$(cd "$SOURCE_CONTENTS/.." && pwd)"

find_repo_root() {
  local dir="$SOURCE_APP"
  while [[ "$dir" != "/" ]]; do
    if [[ -f "$dir/source/AppifyHost/Package.swift" ]]; then
      printf '%s\n' "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
  done

  echo "Could not find repo root containing source/AppifyHost" >&2
  return 1
}

REPO_ROOT="$(find_repo_root)"
APPIFY_HOST_ROOT="$REPO_ROOT/source/AppifyHost"
APP_SERVER_SOURCE="$SOURCE_APP/Contents/Resources/AppServer"
DEVELOPER_SOURCE="$SOURCE_APP/Contents/Developer"

APP="${TW_APP_OUTPUT:-$DEVELOPER_ROOT/dist/$APP_NAME.app}"
SIGN_ADHOC="${TW_APP_SIGN:-1}"

source_hash() {
  local source_dir="$1"
  (
    cd "$source_dir"
    find . -path "./.build" -prune -o -type f \( -name "Package.swift" -o -name "*.swift" \) -print \
      | LC_ALL=C sort \
      | while IFS= read -r source_file; do
          printf '%s\n' "$source_file"
          shasum -a 256 "$source_file" | awk '{print $1}'
        done
  ) | shasum -a 256 | awk '{print $1}'
}

if [[ ! -f "$APPIFY_HOST_ROOT/Package.swift" ]]; then
  echo "Missing AppifyHost source at $APPIFY_HOST_ROOT" >&2
  exit 1
fi

if [[ ! -x "$APP_SERVER_SOURCE/main.sh" ]]; then
  echo "Missing tw app server at $APP_SERVER_SOURCE" >&2
  exit 1
fi

if [[ ! -d "$DEVELOPER_SOURCE/Scripts" ]]; then
  echo "Missing tw developer scripts at $DEVELOPER_SOURCE/Scripts" >&2
  exit 1
fi

STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/tw-build.XXXXXX")"
cleanup() {
  rm -rf "$STAGING_ROOT"
}
trap cleanup EXIT

APP_PAYLOAD="$STAGING_ROOT/AppPayload"
mkdir -p "$APP_PAYLOAD/Contents/Resources" "$APP_PAYLOAD/Contents/Developer"
rsync -a --delete "$APP_SERVER_SOURCE/" "$APP_PAYLOAD/Contents/Resources/AppServer/"
rsync -a --delete \
  --exclude "dist" \
  "$DEVELOPER_SOURCE/" "$APP_PAYLOAD/Contents/Developer/"

swift build --package-path "$APPIFY_HOST_ROOT" -c release --product appify-host
APPIFY_HOST_SOURCE_HASH="$(source_hash "$APPIFY_HOST_ROOT")"

CONTENTS="$APP/Contents"
MACOS="$CONTENTS/MacOS"
RESOURCES="$CONTENTS/Resources"
DEVELOPER="$CONTENTS/Developer"
APPIFY_HOST_SOURCE="$RESOURCES/AppifyHostSource"
APP_SERVER="$RESOURCES/AppServer"

rm -rf "$APP"
mkdir -p "$MACOS" "$RESOURCES" "$DEVELOPER"

cp "$APPIFY_HOST_ROOT/.build/release/appify-host" "$MACOS/appify-host"
chmod +x "$MACOS/appify-host"

cp "$APPIFY_HOST_ROOT/Scripts/main.sh" "$MACOS/main.sh"
chmod +x "$MACOS/main.sh"

mkdir -p "$APPIFY_HOST_SOURCE"
rsync -a --delete --exclude ".build" "$APPIFY_HOST_ROOT/" "$APPIFY_HOST_SOURCE/"
printf '%s\n' "$APPIFY_HOST_SOURCE_HASH" > "$APPIFY_HOST_SOURCE/.appify-host-source-hash"
printf '%s\n' "$APPIFY_HOST_SOURCE_HASH" > "$MACOS/.appify-host-binary-source-hash"

mkdir -p "$APP_SERVER"
rsync -a --delete "$APP_PAYLOAD/Contents/Resources/AppServer/" "$APP_SERVER/"

rsync -a --delete "$APP_PAYLOAD/Contents/Developer/" "$DEVELOPER/"

cat > "$CONTENTS/Info.plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>CFBundleDevelopmentRegion</key>
  <string>en</string>
  <key>CFBundleDisplayName</key>
  <string>$APP_NAME</string>
  <key>CFBundleExecutable</key>
  <string>$EXECUTABLE_NAME</string>
  <key>CFBundleIdentifier</key>
  <string>$BUNDLE_IDENTIFIER</string>
  <key>CFBundleInfoDictionaryVersion</key>
  <string>6.0</string>
  <key>CFBundleName</key>
  <string>$APP_NAME</string>
  <key>CFBundlePackageType</key>
  <string>APPL</string>
  <key>CFBundleShortVersionString</key>
  <string>$VERSION</string>
  <key>CFBundleSupportedPlatforms</key>
  <array>
    <string>MacOSX</string>
  </array>
  <key>CFBundleVersion</key>
  <string>$VERSION</string>
  <key>LSMinimumSystemVersion</key>
  <string>14.0</string>
  <key>NSHighResolutionCapable</key>
  <true/>
  <key>NSHumanReadableCopyright</key>
  <string>Copyright © 2026 subtleGradient</string>

  <key>AppifyHost</key>
  <dict>
    <key>DocumentMode</key>
    <string>fileDocument</string>
    <key>ServerInstallDirectory</key>
    <string>Contents/Resources/AppServer</string>
    <key>ServerExecutable</key>
    <string>main.sh</string>
    <key>ServerArguments</key>
    <array/>
    <key>DocumentKindEnvironmentValue</key>
    <string>com.subtlegradient.tw.csv</string>
    <key>FirstLaunchHelp</key>
    <dict>
      <key>URL</key>
      <string>https://github.com/shshemi/tabiew#useful-keybindings%EF%B8%8F</string>
      <key>WindowTitle</key>
      <string>Tabiew Keybindings</string>
    </dict>
    <key>LogName</key>
    <string>tw</string>
    <key>WindowTitlePrefix</key>
    <string>tw</string>
    <key>StartupTimeoutSeconds</key>
    <integer>600</integer>
    <key>EnvironmentVariables</key>
    <dict>
      <key>TW_DOCUMENT</key>
      <string>{documentPath}</string>
    </dict>
    <key>WebViewDataStore</key>
    <string>nonPersistent</string>
  </dict>

  <key>UTImportedTypeDeclarations</key>
  <array>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.csv</string>
      <key>UTTypeDescription</key>
      <string>CSV Table</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>csv</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.tsv</string>
      <key>UTTypeDescription</key>
      <string>TSV Table</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>tsv</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.parquet</string>
      <key>UTTypeDescription</key>
      <string>Parquet Data</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>parquet</string>
          <string>pqt</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.json</string>
      <key>UTTypeDescription</key>
      <string>JSON Data</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>json</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.jsonl</string>
      <key>UTTypeDescription</key>
      <string>JSON Lines Data</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>jsonl</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.arrow</string>
      <key>UTTypeDescription</key>
      <string>Arrow Data</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>arrow</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.fwf</string>
      <key>UTTypeDescription</key>
      <string>Fixed-Width Data</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>fwf</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.sqlite</string>
      <key>UTTypeDescription</key>
      <string>SQLite Database</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>db</string>
          <string>sqlite</string>
          <string>sqlite3</string>
        </array>
      </dict>
    </dict>
    <dict>
      <key>UTTypeIdentifier</key>
      <string>com.subtlegradient.tw.excel</string>
      <key>UTTypeDescription</key>
      <string>Excel Workbook</string>
      <key>UTTypeConformsTo</key>
      <array>
        <string>public.data</string>
      </array>
      <key>UTTypeTagSpecification</key>
      <dict>
        <key>public.filename-extension</key>
        <array>
          <string>xls</string>
          <string>xlsx</string>
          <string>xlsm</string>
          <string>xlsb</string>
        </array>
      </dict>
    </dict>
  </array>

  <key>CFBundleDocumentTypes</key>
  <array>
    <dict>
      <key>CFBundleTypeName</key>
      <string>Delimited Table</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>csv</string>
        <string>tsv</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.csv</string>
        <string>com.subtlegradient.tw.tsv</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>Parquet Data</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>parquet</string>
        <string>pqt</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.parquet</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>JSON Data</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>json</string>
        <string>jsonl</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.json</string>
        <string>com.subtlegradient.tw.jsonl</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>Arrow Data</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>arrow</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.arrow</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>Fixed-Width Data</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>fwf</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.fwf</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>SQLite Database</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>db</string>
        <string>sqlite</string>
        <string>sqlite3</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.sqlite</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
    <dict>
      <key>CFBundleTypeName</key>
      <string>Excel Workbook</string>
      <key>CFBundleTypeRole</key>
      <string>Viewer</string>
      <key>CFBundleTypeExtensions</key>
      <array>
        <string>xls</string>
        <string>xlsx</string>
        <string>xlsm</string>
        <string>xlsb</string>
      </array>
      <key>LSHandlerRank</key>
      <string>Alternate</string>
      <key>LSItemContentTypes</key>
      <array>
        <string>com.subtlegradient.tw.excel</string>
      </array>
      <key>NSDocumentClass</key>
      <string>AppifyHostDocument</string>
    </dict>
  </array>
</dict>
</plist>
PLIST

cat > "$CONTENTS/PkgInfo" <<PKGINFO
APPL????
PKGINFO

if [[ "$SIGN_ADHOC" != "1" ]]; then
  cat > "$APP/.gitignore" <<GITIGNORE
Contents/Resources/AppifyHostSource/.build/
Contents/Developer/dist/
GITIGNORE
fi

plutil -lint "$CONTENTS/Info.plist"

if [[ "$SIGN_ADHOC" == "1" ]] && command -v codesign >/dev/null 2>&1; then
  codesign --force --deep --sign - "$APP"
fi

echo "$APP"
