...
async function ensureSchemaFile(canvasFilePath: string): Promise<void> {
  const schemaPath = join(dirname(canvasFilePath), CANVAS_SCHEMA_FILE_NAME);
  if (existsSync(schemaPath)) {
    return;
  }

  const bundledSchemaPath = join(import.meta.dir, "..", "schemas", "tldraw-canvas.schema.json");
  if (existsSync(bundledSchemaPath)) {
    await Bun.write(schemaPath, Bun.file(bundledSchemaPath));
  }
}

async function getCanvasState(canvasFilePath: string): Promise<CanvasStatePayload> {
  await ensureSchemaFile(canvasFilePath);
...
function resolveDocumentPath() {
  ...
  if (!documentPath) {
    throw new Error("Expected a .tldraw document path as the last argument");
  }
