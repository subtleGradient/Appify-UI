...
public struct TuiHostConfiguration: Equatable, Sendable {
    ...
    public var documentMode: TuiHostDocumentMode
    ...
    public init(
        ...
        documentMode: TuiHostDocumentMode,
        commandName: String,
...
public enum TuiHostDocumentMode: String, Equatable, Sendable {
    case folderMarker
    case fileDocument
}
...
public enum TuiHostCoreError: Error, Equatable, CustomStringConvertible, Sendable {
    ...
    public var description: String {
        ...
        case .missingRequirements(let missing):
            "Could not find nix-shell. Also missing direct runtime requirement(s): \(missing.joined(separator: ", ")). Install Nix, or make \(missing.joined(separator: ", ")) available on PATH."
...
public enum TuiHostConfigurationLoader {
    ...
    public static func load(
        ...
        let documentModeValue = stringValue(hostSettings["DocumentMode"]) ?? TuiHostDocumentMode.folderMarker.rawValue
        guard let documentMode = TuiHostDocumentMode(rawValue: documentModeValue) else {
            throw TuiHostCoreError.invalidInfoPlist("Unsupported TuiHost:DocumentMode: \(documentModeValue).")
        }
...
public enum PackageDocument {
    ...
    public static func workingDirectory(forPackage packageURL: URL, configuration: TuiHostConfiguration) throws -> URL {
        ...
        guard standardized.isFileURL else {
            throw TuiHostCoreError.invalidPackage("Expected a local document.")
        }
        ...
        case .folderMarker:
            return try workingDirectoryForFolderMarker(standardized)
        case .fileDocument:
            return try workingDirectoryForFileDocument(standardized)
        }
    }

    private static func workingDirectoryForFolderMarker(_ packageURL: URL) throws -> URL {
        let values: URLResourceValues
        do {
            values = try packageURL.resourceValues(forKeys: [.isDirectoryKey, .isSymbolicLinkKey])
        } catch {
            throw TuiHostCoreError.invalidPackage("\(packageURL.lastPathComponent) is not a readable package folder.")
        }
        guard values.isSymbolicLink != true else {
            throw TuiHostCoreError.invalidPackage("\(packageURL.lastPathComponent) must be a real folder, not a symlink.")
        }
        guard values.isDirectory == true else {
            throw TuiHostCoreError.invalidPackage("\(packageURL.lastPathComponent) is not a folder.")
        }

        return packageURL.deletingLastPathComponent()
    }

    private static func workingDirectoryForFileDocument(_ documentURL: URL) throws -> URL {
        let values: URLResourceValues
        do {
            values = try documentURL.resourceValues(forKeys: [.isRegularFileKey, .isSymbolicLinkKey])
        } catch {
            throw TuiHostCoreError.invalidPackage("\(documentURL.lastPathComponent) is not a readable document.")
        }
        guard values.isSymbolicLink != true else {
            throw TuiHostCoreError.invalidPackage("\(documentURL.lastPathComponent) must be a real file, not a symlink.")
        }
        guard values.isRegularFile == true else {
            throw TuiHostCoreError.invalidPackage("\(documentURL.lastPathComponent) is not a file.")
        }
...
public struct TerminalToolResolver: Sendable {
    ...
    private func localToolDirectories() -> [String] {
        let directories = ["/usr/local/bin"]
        let optURL = URL(fileURLWithPath: "/opt", isDirectory: true)
        let options: FileManager.DirectoryEnumerationOptions = [.skipsHiddenFiles, .skipsPackageDescendants]
        guard let optEntries = try? FileManager.default.contentsOfDirectory(
            at: optURL,
        ...
        let optBinDirectories = optEntries
            .sorted { $0.path < $1.path }
            .compactMap { entry -> String? in
                guard (try? entry.resourceValues(forKeys: [.isDirectoryKey]).isDirectory) == true else {
                    return nil
                }
                return entry.appendingPathComponent("bin", isDirectory: true).path
            }

        return directories + optBinDirectories
    }
}
