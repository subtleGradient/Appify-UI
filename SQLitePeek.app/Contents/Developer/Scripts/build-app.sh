...
SIGN_ADHOC="${SQLITE_PEEK_APP_SIGN:-1}"
...
if [[ ! -d "$DEVELOPER_SOURCE/Scripts" ]]; then
  echo "Missing SQLite Peek developer scripts at $DEVELOPER_SOURCE/Scripts" >&2
  exit 1
fi

STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/sqlite-peek-build.XXXXXX")"
cleanup() {
  rm -rf "$STAGING_ROOT"
}
trap cleanup EXIT
