...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @objc private func openDocumentFromMenu(_ sender: Any?) {
        DispatchQueue.main.async { [weak self] in
            self?.showExistingDocumentPicker()
        }
    }

    @MainActor
    private func showNewDocumentPicker() {
    ...
    @MainActor
    private func showExistingDocumentPicker() {
        guard let configuration else {
            return
        }

        let panel = NSOpenPanel()
        panel.title = openPanelTitle(configuration: configuration)
    ...
    private func configureMainMenu() {
        ...
        let newItem = fileMenu.addItem(
            withTitle: newWindowMenuTitle(configuration: configuration),
        ...
        let openItem = fileMenu.addItem(
            withTitle: openMenuTitle(configuration: configuration),
    ...
    private func openPanelTitle(configuration: TuiHostConfiguration) -> String {
        switch configuration.documentMode {
        case .folderMarker:
            return "Open \(configuration.appName) Package"
        case .fileDocument:
            return "Open \(configuration.appName) Document"
        }
    }

    private func newWindowMenuTitle(configuration: TuiHostConfiguration) -> String {
        switch configuration.documentMode {
        case .folderMarker:
            return "New \(configuration.windowTitlePrefix) Window..."
        case .fileDocument:
            return "Open \(configuration.windowTitlePrefix) Document..."
        }
    }

    private func openMenuTitle(configuration: TuiHostConfiguration) -> String {
        switch configuration.documentMode {
        case .folderMarker:
            return "Open Package..."
        case .fileDocument:
