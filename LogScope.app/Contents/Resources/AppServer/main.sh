...
APP_NAME="${APPIFY_HOST_APP_NAME:-LogScope}"
...
start_with_nix() {
  ...
  local base_path="$3"
  local command
  command="exec ttyd --interface 127.0.0.1 --port $(shell_quote "$port") --writable --check-origin --once --max-clients 1 --base-path $(shell_quote "$base_path") --cwd $(shell_quote "$WORKING_DIRECTORY") lnav $(shell_quote "$DOCUMENT_PATH")"
  "$nix_shell" -p ttyd lnav --run "$command" &
}
...
start_direct() {
  ...
  lnav="$(find_tool lnav)" || return 1
  export PATH="$(dirname "$lnav"):$(dirname "$ttyd"):${PATH:-}"
  "$ttyd" \
    --interface 127.0.0.1 \
    --port "$port" \
    --writable \
...
if NIX_SHELL="$(find_tool nix-shell)"; then
  ...
  if ! start_direct "$PORT" "$BASE_PATH"; then
    printf '%s requires nix-shell or direct installations of ttyd and lnav.\n' "$APP_NAME" >&2
    exit 1
  fi
  CHILD_PID="$!"
fi
