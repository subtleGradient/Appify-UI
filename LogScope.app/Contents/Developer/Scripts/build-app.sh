...
BUNDLE_IDENTIFIER="com.subtlegradient.logscope"
VERSION="0.1.0"
...
APP="${LOGSCOPE_APP_OUTPUT:-$DEVELOPER_ROOT/dist/$APP_NAME.app}"
SIGN_ADHOC="${LOGSCOPE_APP_SIGN:-1}"
...
if [[ ! -x "$APP_SERVER_SOURCE/main.sh" ]]; then
  echo "Missing LogScope app server at $APP_SERVER_SOURCE" >&2
  exit 1
fi

if [[ ! -d "$DEVELOPER_SOURCE/Scripts" ]]; then
  echo "Missing LogScope developer scripts at $DEVELOPER_SOURCE/Scripts" >&2
  exit 1
fi

STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/logscope-build.XXXXXX")"
cleanup() {
  rm -rf "$STAGING_ROOT"
}
trap cleanup EXIT
...
<dict>
  ...
  <dict>
    ...
    <dict>
      <key>URL</key>
      <string>https://docs.lnav.org/en/latest/hotkeys.html</string>
      <key>WindowTitle</key>
      <string>lnav Hotkeys</string>
    </dict>
    <key>LogName</key>
    <string>LogScope</string>
