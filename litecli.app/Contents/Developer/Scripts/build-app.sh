...
BUNDLE_IDENTIFIER="com.subtlegradient.litecli"
VERSION="0.1.0"
...
APP="${LITECLI_APP_OUTPUT:-$DEVELOPER_ROOT/dist/$APP_NAME.app}"
SIGN_ADHOC="${LITECLI_APP_SIGN:-1}"
...
if [[ ! -x "$APP_SERVER_SOURCE/main.sh" ]]; then
  echo "Missing litecli app server at $APP_SERVER_SOURCE" >&2
  exit 1
fi

if [[ ! -d "$DEVELOPER_SOURCE/Scripts" ]]; then
  echo "Missing litecli developer scripts at $DEVELOPER_SOURCE/Scripts" >&2
  exit 1
fi

STAGING_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/litecli-build.XXXXXX")"
cleanup() {
  rm -rf "$STAGING_ROOT"
}
trap cleanup EXIT
...
<dict>
  ...
  <dict>
    ...
    <string>com.subtlegradient.litecli.sqlite</string>
    <key>LogName</key>
    <string>litecli</string>
